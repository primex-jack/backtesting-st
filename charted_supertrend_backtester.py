...
# Script Version
SCRIPT_VERSION = "2.10.15"  # Updated for SyntaxError fix in log writing
...
def backtest_strategy(df, atr_period, atr_ratio, start_date, end_date, verbose=False, quiet=False):
    ...
    # Initialize backtest_data DataFrame with dtypes
    backtest_data = pd.DataFrame({
        'timestamp': pd.Series(dtype='datetime64[ns]'),
        'trading_pair': pd.Series(dtype='object'),
        ...
        'profit_per_contract': pd.Series(dtype='float64'),
        'normalized_profit': pd.Series(dtype='float64'),
        'cumulative_profit': pd.Series(dtype='float64'),
        'floating_pl': pd.Series(dtype='float64'),
    ...
    for i in range(1, len(df)):
        ...
        # Calculate account value for this candle
        row = {
            'timestamp': timestamp,
            'trading_pair': args.symbol,
            'timeframe': args.timeframe,
            'trade_type': 'None',
            'entry_price': last_entry_price if position != 0 else 0,
            'exit_price': close,
            'size': args.trade_size / last_entry_price if position != 0 and last_entry_price != 0 else 0,
            'profit_per_contract': 0,
            'normalized_profit': 0,
            'cumulative_profit': cumulative_profit,
            'floating_pl': 0,
            'account_value': args.starting_capital + cumulative_profit
        }
        if position != 0 and last_entry_price != 0:
            contracts = args.trade_size / last_entry_price
            row['floating_pl'] = (close - last_entry_price) * contracts if position == 1 else (last_entry_price - close) * contracts
            row['account_value'] = args.starting_capital + cumulative_profit + row['floating_pl']
        ...
        if start_date <= timestamp <= end_date:
            ...
            if position == 0:
                if long_condition:
                    ...
                    row['size'] = args.trade_size / close
                ...
                if short_condition:
                    ...
                    max_runup = (trade_period['high'] - last_entry_price).max() if not trade_period.empty else 0.0
                    lowest_price = trade_period['low'].min() if not trade_period.empty else close
                    max_drawdown = max(last_entry_price - lowest_price, 0)
                    if not quiet and not trade_period.empty:
                    ...
                    contracts = args.trade_size / last_entry_price
                    normalized_profit = profit * contracts
                    total_profit += profit
                    cumulative_profit += normalized_profit
                    if not quiet:
                    ...
                    # Update row for trade exit and new short entry
                    row['trade_type'] = 'LONG_to_SHORT'
                    row['entry_price'] = last_entry_price
                    row['exit_price'] = close
                    row['size'] = contracts
                    row['profit_per_contract'] = profit
                    row['normalized_profit'] = normalized_profit
                    row['cumulative_profit'] = cumulative_profit
                    row['floating_pl'] = 0
                    row['account_value'] = args.starting_capital + cumulative_profit
                    # New short position
                    position = -1
                    last_entry_price = close
                    last_entry_timestamp = timestamp
                    ...
                    # Update row to reflect new short position for next candle
                    row['entry_price'] = close
                    row['size'] = args.trade_size / close
                    backtest_data = pd.concat([backtest_data, pd.DataFrame([row])], ignore_index=True)
                    continue
            elif position == -1:
                if long_condition:
                    if not quiet:
                    ...
                    trade_period = df[(df.index >= last_entry_timestamp) & (df.index <= timestamp)]
                    max_runup = (last_entry_price - trade_period['low']).max() if not trade_period.empty else 0.0
                    max_drawdown = (trade_period['high'] - last_entry_price).max() if not trade_period.empty else 0.0
                    if not quiet and not trade_period.empty:
                    ...
                    # Update row for trade exit and new long entry
                    row['trade_type'] = 'SHORT_to_LONG'
                    row['entry_price'] = last_entry_price
                    row['exit_price'] = close
                    row['size'] = contracts
                    row['profit_per_contract'] = profit
                    ...
                    # Update row to reflect new long position for next candle
                    row['entry_price'] = close
                    row['size'] = args.trade_size / close
                    backtest_data = pd.concat([backtest_data, pd.DataFrame([row])], ignore_index=True)
                    continue
    ...
    if position != 0:
        ...
        contracts = args.trade_size / last_entry_price if last_entry_price != 0 else 0
        unrealized_pl = (last_close - last_entry_price) * contracts if position == 1 else (last_entry_price - last_close) * contracts
        max_runup = (trade_period['high'] - last_entry_price).max() if position == 1 and not trade_period.empty else (last_entry_price - trade_period['low']).max() if not trade_period.empty else 0.0
        max_drawdown = max(last_entry_price - trade_period['low'].min(), 0) if position == 1 and not trade_period.empty else (trade_period['high'] - last_entry_price).max() if not trade_period.empty else 0.0
        if not quiet and not trade_period.empty:
            print(f"{'Lowest' if position == 1 else 'Highest'} price during open trade: {trade_period['low' if position == 1 else 'high'].min() if position == 1 else trade_period['high'].max()} at {trade_period['low' if position == 1 else 'high'].idxmin() if position == 1 else trade_period['high'].idxmax()}")
        trades.append({
            'type': 'open',
        ...
        # Update last row for open trade
        row = {
            'timestamp': df.index[i - 1],
            'trading_pair': args.symbol,
            'timeframe': args.timeframe,
            ...
            'normalized_profit': 0,
            'cumulative_profit': cumulative_profit,
            'floating_pl': unrealized_pl,
            'account_value': args.starting_capital + cumulative_profit + unrealized_pl
        }
        backtest_data = pd.concat([backtest_data, pd.DataFrame([row])], ignore_index=True)

    # Validate for duplicate timestamps
    duplicates = backtest_data['timestamp'].duplicated().sum()
    if duplicates > 0:
        print(f"Warning: Found {duplicates} duplicate timestamps in backtest_data. Keeping last row per timestamp.")
        backtest_data = backtest_data.drop_duplicates(subset='timestamp', keep='last')

    if not quiet:
        print("backtest_strategy completed")
    return trades, total_profit, df, backtest_data
...
def run_single_combination(df, atr_period, atr_ratio, start_date, end_date, symbol, timeframe, quiet=False):
    if not quiet:
        print(f"Starting run_single_combination with atr_period: {atr_period}, atr_ratio: {atr_ratio}")
    trades, total_profit, df_with_indicators, backtest_data = backtest_strategy(df.copy(), atr_period, atr_ratio, start_date, end_date, quiet=quiet)
    if stop_flag:
        if not quiet:
    ...
    return atr_period, atr_ratio, metrics, backtest_data

def main():
    global args  # Make args global for backtest_strategy
    ...
    parser.add_argument('--starting-capital', type=float, default=100000.0, help='Starting capital (default: 100000.0)')
    parser.add_argument('--trade-size', type=float, default=100000.0, help='Trade size (default: 100000.0)')
    ...
    timeframe_map = {'1m': 1, '3m': 3, '5m': 5, '15m': 15, '30m': 30, '1h': 60, '4h': 240, '1d': 1440}
    ...
    if args.atr_period_range and args.atr_ratio_range:
        ...
        with tqdm(total=total_combinations, desc="Testing combinations", file=sys.stdout) as pbar:
            with concurrent.futures.ProcessPoolExecutor(max_workers=args.workers) as executor:
                futures = [
                    executor.submit(run_single_combination, df.copy(), period, ratio, start_date, end_date, args.symbol, args.timeframe, args.quiet)
                ...
                for future in concurrent.futures.as_completed(futures):
                    if stop_flag:
                        ...
                        break
                    try:
                        atr_period, atr_ratio, metrics, _ = future.result()
                        if metrics:
        ...
        if args.verbose and not stop_flag and results:
            ...
            print_range_summary(results, args.symbol, args.timeframe, args.date_start, args.date_end, args.starting_capital, args.top_n)
        if not stop_flag:
            print_range_summary(results, args.symbol, args.timeframe, args.date_start, args.date_end, args.starting_capital, args.top_n)
        else:
        ...
        trades, total_profit, df_with_indicators, backtest_data = backtest_strategy(df, args.atr_period, args.atr_ratio, start_date, end_date)
        if not stop_flag:
            print_trade_summary(trades, total_profit, args.symbol, args.timeframe, args.date_start, args.date_end, args.atr_period, args.atr_ratio, args.starting_capital)
            # Save CSV
            output_dir = os.path.expanduser("~/backtest_project/account_value_reports")
            os.makedirs(output_dir, exist_ok=True)
            csv_filename = f"{args.symbol}_{args.timeframe}_atr{args.atr_period}_ratio{args.atr_ratio}.csv"
            backtest_data.to_csv(os.path.join(output_dir, csv_filename), index=False)
            logger.info(f"Saved backtest CSV to {os.path.join(output_dir, csv_filename)}")
            # Save log
            output_dir_logs = os.path.expanduser("~/backtest_project/single_reports")
            os.makedirs(output_dir_logs, exist_ok=True)
            log_filename = f"{args.symbol}_{args.timeframe}_atr{args.atr_period}_ratio{args.atr_ratio}.log"
            with open(os.path.join(output_dir_logs, log_filename), 'w') as f:
                f.write(f"Backtest for {args.symbol}, {args.timeframe}, ATR Period={args.atr_period}, ATR Ratio={args.atr_ratio}\n")
                f.write("=" * 80 + "\n")
                ...
                f.write("=" * 80 + "\n")
            logger.info(f"Saved backtest log to {os.path.join(output_dir_logs, log_filename)}")
            if args.chart:
                ...
                plot_supertrend_chart(df_with_indicators, trades, args.date_start, args.date_end, args.symbol, args.timeframe, output_filename=output_filename)
        else:
