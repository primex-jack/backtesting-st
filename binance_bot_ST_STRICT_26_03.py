...
SCRIPT_VERSION = "2.8.1"  # Updated to 2.8.1 for bug fix: added retry logic to handle connection errors in stop-loss updates
...
def init_db():
    ...
    if binance_position:
        ...
        c.execute(
            "UPDATE trades SET exit_price = NULL, side = ?, entry_price = ?, size = ?, stop_loss = ?, stop_loss_order_id = ?, order_id = ? WHERE trading_pair = ? AND exit_price IS NULL",
            (current_position['side'], current_position['entry_price'], current_position['size'],
             current_position.get('stop_loss'), current_position.get('stop_loss_order_id'), None, TRADING_PAIR)
        )
        if c.rowcount == 0:
            c.execute(
                ...
                (str(datetime.now(timezone.utc)), TRADING_PAIR, TIMEFRAME, current_position['side'],
                 ...
                 current_position.get('stop_loss_order_id'), None)
            )
        conn.commit()
    else:
        c.execute("SELECT * FROM trades WHERE exit_price IS NULL AND trading_pair = ?", (TRADING_PAIR,))
        open_trade = c.fetchone()
        if open_trade:
            current_position = {
                'side': open_trade[4],
                'entry_price': open_trade[5],
                'size': open_trade[6],
                'stop_loss': open_trade[8],
                'stop_loss_order_id': open_trade[12],
                'order_id': open_trade[11],
                'open_time': open_trade[1]
            }
            logger.info(
                f"Loaded open position from database: Side: {current_position['side']}, Entry Price: {current_position['entry_price']:.2f}, Stop Loss: {current_position.get('stop_loss', 'None')}")
    return conn
...
def display_trade_summary(position, latest_close, line_st):
    if position:
        ...
        logger.info(
            f"{Fore.CYAN}Open Trade Summary - Side: {position['side']}, Open Time: {position['open_time']}, Entry Price: {position['entry_price']:.4f}, "
...
def fetch_historical_data(symbol=TRADING_PAIR, interval=TIMEFRAME, limit=50, end_time=None):
    ...
    try:
        ...
        kline_data = pd.concat([historical_data, kline_data], ignore_index=True)
        kline_data.sort_values('timestamp', inplace=True)
        kline_data.drop_duplicates(subset='timestamp', keep='last', inplace=True)
        kline_data = kline_data.tail(1000)
        logger.info(
            f"{Fore.CYAN}Fetched {len(klines)} historical candles. Added {len(kline_data) - prev_rows} new rows.{Style.RESET_ALL}")
    ...
    try:
        ...
        if 'data' in data and 'k' in data['data']:
            ...
            if not first_closed_candle_received and kline['x']:
                ...
                #fetch_historical_data(end_time=int(first_closed_timestamp) - 1)
                fetch_limit = min(max(10 * ATR_PERIOD, 50), 1000)  # Caps at Binance API limit of 1000
                fetch_historical_data(limit=fetch_limit, end_time=int(first_closed_timestamp) - 1)
            ...
            if kline['x']:
                if len(kline_data) >= ATR_PERIOD:
                    ...
                    # Existing trade logic continues...
                else:
                    logger.warning(
